...
else
{
  ...
  $.ajax({
type: "POST",
url: "php/create1_salary.php", 

data: {name:$("#name").val(),f_name:$("#f_name").val(),email:$("#email").val(), number:$("#phone").val(),job_title:$("#job-title").val(), date_of_joining:$("#date1").val(),gross_salary:$("#gross_salary").val(),net_pay:$("#net_pay").val(),reg_id:$("#registration_id").val(),date_of_printing:$("#date-of-printing").val(),certificate_category:$("#certificate_category").val()},
beforeSend: function() {
$("#loading").fadeIn();
},
